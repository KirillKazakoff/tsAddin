...
export const getExportContractCells = (agreement: ExportGroupT) => {
    ...
    const cells = {
        ...
        cfr: [
            ...
            {
                name: 'Доставка_порт',
                eng: `3.5 The delivery of goods to Buyer, mentioned in clause 1 of this Agreement should be carried in port of destination ${portTo.eng.name}, ${portTo.eng.country} no later than ${date.delivery('eng', 'month')}`,
                ru: `3.5 Передача Покупателю Товара, оговоренного в п.1 настоящего Дополнения будет производиться в порту назначения ${portTo.ru.name}, ${portTo.ru.country} не позднее чем ${date.delivery('ru', 'month')}`,
                height: 40,
            },
            {
        ...
        fcaCom: [
            ...
            //     ru: `3.1 Поставка осуществлялась на условиях FCA ${portTo.ru.name} в рамках Дополнения № ${storageAgreementRow?.agreementNo} от ${date.storageAgreementDate('ru')} к контракту купли-продажи ${storageAgreementRow?.contract?.contractNo} от ${date.contract('ru')} Приемка-передача Товара по количеству и качеству производилась на территории: ${portTo.ru.countryFull} в п. ${portTo.ru.name}.`,
            //     height: 70,
            // },
            {
            ...
            {
                name: 'Доставка_дата',
                eng: `Discharge date: ${deliveryDate}`,
        ...
        exportStorage: [
            ...
            {
                name: 'Покупатель_шапка_заголовок',
                eng: 'Contractor:',
                ru: 'Исполнитель:',
            },
            {
                name: 'Продавец_заголовок',
                eng: 'Customer:',
                ru: 'Заказчик:',
            },
            {
                name: 'Покупатель_заголовок',
                eng: 'Contractor:',
                ru: 'Исполнитель:',
            },
            {
