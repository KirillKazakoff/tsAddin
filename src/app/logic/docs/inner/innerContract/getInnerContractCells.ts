    ...
    const cells = {
        common: [
            ...
            { cell: 'Договор_дата', value: date.contract },
            {
                ...
                value: {
                    richText: [
                        ...
                        richText({ text: `именуемое в дальнейшем «Поставщик», ${podpisant.req.face}, действущ${podpisant.req.sex} ${podpisant.req.base}, с одной стороны, и ` }),
            ...
            },
            {
                ...
                value: {
                    richText: [
                        richText({ text: `4.2. Товар доставляется в порт ${r.port.code} ${r.terms === 'CFR(Контейнер)' ? 'из порта Магадан в рефрижераторном(ых) контейнере(ах)' : ''} морским транспортом ` }),
                        richText({ text: `(${r.transport.ru.name}${r.terms === 'CFR(Контейнер)' ? `, рейс ${mr.reice}` : ''}), `, isBold: true }),
            ...
            },
            {
                ...
                value: r.terms === 'CFR(Контейнер)' ? `4.3. Товар передается Покупателю в рефрижераторном(ых) контейнере(ах) на контейнерном терминале в порту ${r.port.code} по товарной накладной после полной оплаты стоимости товара в соответствии с п. 4.1 договора. О точной дате передачи товара Поставщик дополнительно извещает Покупателя письмом. Со дня, следующего за указанным в письме Поставщика днем передачи товара, расходы по подключению и электропитанию контейнера(ов), а также риски, связанные с несвоевременным подключением контейнеров, несет Покупатель. Право собственности на товар переходит к Покупателю с момента оформления Поставщиком ВСЭ.` : `4.3. Товар передается Покупателю на борту транспортного судна, доставившего товар в порт ${r.port.code}. Право собственности на товар переходит к Покупателю с момента оформления Продавцом ВСЭ.`,
                height: r.terms === 'CFR(Контейнер)' ? 70 : 30,
            },
            { cell: 'Договор_приемка_товара', value: `4.4. Приемка товара по качеству и количеству осуществляется Покупателем или его уполномоченным представителем в порту ${r.port.code} в следующем порядке:` },
            { cell: 'Договор_приемка_товара_количество', value: `▪ по количеству – в момент получения товара в порту ${r.port.code}` },
            { cell: 'Договор_приемка_товара_качество', value: `▪ по качеству – в течение 3-х дней с момента с момента получения товара в порту ${r.port.code}.` },
            { cell: 'Договор_приемка_товара_ВСД', value: r.terms === 'CFR(Контейнер)' ? '4.6. Ветеринарные сопроводительные документы (ВСД) необходимые для транспортировки товара из г. Магадана до контейнерного терминала, расположенного по адресу: г. Владивосток, ул. Морозова, 7А, оформляются Поставщиком.  После прибытия товара в контейнерный терминал, указанный в настоящем пункте, оформление ВСД на товар осуществляет Покупатель своими силами и за свой счет. ' : '4.6. Ветеринарные сопроводительные документы (ВСД) необходимые для транспортировки товара до порта назначения оформляются Поставщиком. Дальнейшее оформление ВСД на товар осуществляет Покупатель своими силами и за свой счет.', height: r.terms === 'CFR(Контейнер)' ? 40 : 30 },
        ],
        addresses: [
            { cell: 'Договор_адреса_поставщик', value: `Поставщик: ООО «${r.seller.ru.shortName}»` },
            ...
            { cell: 'Договор_адреса_покупатель', value: `Покупатель: ${r.buyer.req.org.form.code} «${r.buyer.name}»` },
