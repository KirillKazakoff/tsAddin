...
export const initRequestTmp = (book: Workbook, contract: ContractT) => {
    ...
    const cells: CellObjT[] = [
        ...
        { cell: 'Прибытие', value: ` * Поставка товара транспортом ${record.transport.ru.name} (рейс №${reiceNo}) ориентировочно ${getExcelDateNumeric(record.deliveryDate, 'ru')} (Зафрахтован Продавцом).` },
